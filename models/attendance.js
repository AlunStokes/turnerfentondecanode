...
attendance.getLatestSession = function(callback) {
  db.pool.query("SELECT id, optionA, optionB, optionC, optionD, answer, startTime, endTime, DATE_FORMAT(startTime, '%d %M, %Y') AS date, createdBy FROM attendancesessions ORDER BY id DESC LIMIT 1;", function(err, rows, fields) {
    if (err) {
...
attendance.getSessionData = function(sessionid, callback) {
  db.pool.query("SELECT id, optionA, optionB, optionC, optionD, answer, startTime, endTime, DATE_FORMAT(startTime, '%d %M, %Y') AS date, createdBy FROM attendancesessions WHERE id = ? ORDER BY id DESC;", [sessionid], function(err, rows, fields) {
    if (err) {
      callback(err);
      return;
    }
...
attendance.getSessionids = function(callback) {
  db.pool.query("SELECT id, DATE_FORMAT(startTime, '%d %M, %Y at %r') AS date FROM attendancesessions ORDER BY id DESC;", function(err, rows, fields) {
    if (err) {
      callback(err);
      return;
    }
...
attendance.getSessionResults = function(sessionid, callback) {
  db.pool.query("SELECT studentNumber, firstName, lastName, decaCluster as cluster FROM members ORDER BY studentNumber", function(err, rows, fields) {
    if (err) {
      callback("Database query error");
      return;
    }
...
attendance.getAbsences = function(studentNumber, callback) {
  db.pool.query("SELECT COUNT(id) AS totalSessions FROM attendancesessions WHERE endTime IS NOT NULL;", function(err, rows, fields) {
    if (err) {
      callback(err);
      return;
    }
    ...
    db.pool.query("SELECT COUNT(correct) AS attended FROM attendancerecords WHERE correct = 1 AND studentNumber = ?;", [studentNumber], function(err, rows, fields) {
      if (err) {
        callback(err);
        return;
      }
