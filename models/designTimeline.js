...
designTimeline.loadPosts = function(callback) {
  ...
  db.pool.query("SELECT entryid, DATE_FORMAT(date, '%Y-%m-%d') AS dateDay, DATE_FORMAT(date, '%M %Y') AS dateMonth, title, author, description, numAlternates FROM designtimelineentries ORDER BY date DESC", function(err, rows, fields) {
    if (err) {
